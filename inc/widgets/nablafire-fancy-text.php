...
class nablafire_fancy_text extends WP_Widget {

	function __construct($defaults, $font_gen) {

		$widget_name 	= esc_html__("Fancy Text",'nablafire-widgets');
		$widget_options = array( 
			'description' => esc_html__("Fancy Text Area", 'nablafire-widgets')
		);
	...
	function widget_settings(){
		...
		$this->settings['title_data']  = new Nablafire_Widget_Data_Setting(
			...
			array(
				'text'		=> array(
					'type'	=> 'text',
					'label'	=> esc_html__('Title Text', 'nablafire-widgets'), 
				...
				'show'		=> array(
					'type'	=> 'checkbox',
					'label'	=> esc_html__('Show Title?', 'nablafire-widgets'), 
					'sanitize' => array('sanitize_data', 'sanitize_pass'),
				),
				'below'		=> array(
					'type'	=> 'checkbox',
					'label'	=> esc_html__('Render Below Text?', 'nablafire-widgets'), 
					'sanitize' => array('sanitize_data', 'sanitize_pass'),
				),
		) );
		...
		$this->settings['text_data']  = new Nablafire_Widget_Data_Setting(
			...
			array(
				...
				'line'		=> array(
					'type'	=> 'number',
					'label'	=> esc_html__('Line Height (%)', 'nablafire-widgets'),
					'desc'	=> __('Set the line height of your text <em>(100-200%)</em>', 
						'nablafire-widgets'),
					'sanitize'	=> array('sanitize_data', 'sanitize_number_range'),
					'atts'		=> array('min'=>100, 'max'=>200),	
				),
				'padding' 	=> array(
					'type'	=> 'text',
					'label' => esc_html__('Padding (CSS Shorthand)', 'nablafire-widgets'), 
				...
				'accent'	=> array(
					'type'	=> 'color',
					'label'	=> esc_html__('Accent Color', 'nablafire-widgets'), 
					'desc'	=> esc_html__('Allows you to highlight your text with 
						the <span> tag', 'nablafire-widgets'),
					'sanitize' => array('sanitize_data', 'sanitize_alpha_color'),
					'atts'	=> $this->defaults['text_mn_accent']
				)
		) );
		...
		$this->settings['div_styles']  = new Nablafire_Widget_Data_Setting(
			array(
				...
				'desc'		=> esc_html__('These values apply to the fancy text container. Note that fancy texts will expand in width to fill their container', 'nablafire-widgets'),
				'color'		=> 'blue'
			),	
			array(
				'color'	=> array(
				...
				'inherit'	=> array(
					'type'	=> 'checkbox',
					'label'	=> esc_html__('Inherit Height?', 'nablafire-widgets'), 
					'desc'	=> __('Inherit the height from the tallest widget in the widegt row?', 
						'nablafire-widgets'),
					'sanitize' => array('sanitize_data', 'sanitize_pass'),
				)
		) );
	...
	function widget($args, $instance) {
		...
		echo $this->write_stylesheet($options); 
		echo $before_widget; ?>

		<div class="wrap-<?php echo esc_html(str_replace('_', '-', $this->id));?>">
			<div class="div-<?php echo esc_html(str_replace('_', '-', $this->id));?>">
			<?php
				if( $options['title_mn_below'] ) {
					$this->widget_text_template($options);
					$this->widget_title_template($options);
				}		
				else {
					$this->widget_title_template($options);
					$this->widget_text_template($options);
	...
	function widget_title_template($options){ ?>
		...
		<?php if( $options['title_mn_text'] && $options['title_mn_show']): ?>
	...
	function widget_text_template($options){ ?>

		<?php if( $options['text_mn_text'] ): ?>
			<div class="d-<?php echo esc_html(str_replace('_', '-', $this->id));?>">
			<?php echo $this->sanitize_data->sanitize_kses( $options, 'text_mn_text' );?></div>
		<?php endif; ?>	
	...
	function write_stylesheet($instance){ 
		...
		$css .= $_css->begin_rule('.wrap-' . str_replace('_', '-', $this->id) );
			...
			if ( $instance['div_mn_inherit'] === true ){
		...
		$css .= $_css->begin_rule('.p-' . str_replace('_', '-', $this->id) );
			...
			$css .= $_css->add_rule('padding'		, $instance['title_mn_padding']);
		...
		$css .= $_css->begin_rule('.d-' . str_replace('_', '-', $this->id) );
			...
			$css .= $_css->add_rule('line-height'	, $instance['text_mn_line'].'%');			
		$css .= $_css->end_rule();

		$css .= $_css->begin_rule('.d-' . str_replace('_', '-', $this->id) . ' span');
			$css .= $_css->add_rule('color'			, $instance['text_mn_accent']);
		$css .= $_css->end_rule();

		$css .= $_css->begin_rule('.div-' . str_replace('_', '-', $this->id) . ' ul');
